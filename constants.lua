...
Constants = {
  ...
  SERVER_ENV_CLOUD = "cloud",
  SERVER_ENV_DEFAULT = "cloud",
  SERVER_HTTP_BASE_URL_LOCAL = "http://127.0.0.1:8787",
  -- Deprecated (legacy only): 클라이언트 런타임은 WS 경로를 사용하지 않는다.
  SERVER_WS_BASE_URL_LOCAL = "ws://127.0.0.1:8787",
  SERVER_HTTP_BASE_URL_CLOUD = "http://project-r-v2.minwlim72.workers.dev",
  -- Deprecated (legacy only): 클라이언트 런타임은 WS 경로를 사용하지 않는다.
  SERVER_WS_BASE_URL_CLOUD = "wss://project-r-v2.minwlim72.workers.dev",
  -- Deprecated (legacy only): WS 비보안 폴백 URL.
  SERVER_WS_BASE_URL_CLOUD_INSECURE = "ws://project-r-v2.minwlim72.workers.dev",
  -- 레거시 기본값 (하위 호환). 실제 요청 URL은 net/server_env.lua 경로를 사용한다.
  SERVER_HTTP_BASE_URL = "http://project-r-v2.minwlim72.workers.dev",
  -- Deprecated (legacy only): 클라이언트 런타임은 WS 기본값을 사용하지 않는다.
  SERVER_WS_BASE_URL = "wss://project-r-v2.minwlim72.workers.dev",
  ...
  PHASE_CHARACTER_SELECT = "CHARACTER_SELECT",
  ...
  TURN_TIME_LIMIT_SEC = readNumberRule("TURN_TIME_LIMIT_SEC", 20),
  SNAPSHOT_TIMEOUT_SEC = readNumberRule("SNAPSHOT_TIMEOUT_SEC", 8),

  MAX_SHOT_POWER = readNumberRule("MAX_SHOT_POWER", 1500),
  ...
  PHYSICS_DAMPING_PER_SEC = readNumberRule("PHYSICS_DAMPING_PER_SEC", 1.80),
  ...
  -- true면 스킬 컷신 시작/종료/스킵 흐름을 콘솔 로그로 출력한다.
  CUTSCENE_DEBUG_LOG = false,
  ...
  -- 스킬 컷신(카드 사용 직후 연출) 공통 튜닝 값.
  CUTSCENE_DEFAULT_DURATION_MS = 3300,
  CUTSCENE_CARD_FOCUS_SEC = 1.02,
  CUTSCENE_BAND_ENTER_SEC = 0.51,
  CUTSCENE_CHARACTER_ENTER_SEC = 0.33,
  CUTSCENE_CHARACTER_SHAKE_SEC = 0.27,
  CUTSCENE_CHARACTER_EXIT_SEC = 0.36,
  CUTSCENE_BAND_EXIT_SEC = 0.42,
  CUTSCENE_DIM_ALPHA = 0.34,
  CUTSCENE_BAND_WIDTH = 1460,
  CUTSCENE_BAND_HEIGHT = 178,
  CUTSCENE_BAND_TILT_PX = 38,
  CUTSCENE_BAND_SHAKE_PX = 7,
  CUTSCENE_BAND_REAR_HEIGHT_SCALE = 0.74,
  CUTSCENE_BAND_FRONT_HEIGHT_SCALE = 1.08,
  CUTSCENE_CHARACTER_W = 300,
  CUTSCENE_CHARACTER_H = 300,
  CUTSCENE_SKIP_BUTTON_W = 130,
  CUTSCENE_SKIP_BUTTON_H = 40,
  ...
  -- 충전 시스템 (초능력) — shared/characters.json 글로벌 섹션에서 로드
  CHARGE_PER_TURN = 0.25,
  CHARGE_ON_ALLY_OUT = 0.15,
  CHARGE_MAX = 1.0,

  -- 캐릭터별 알 색상 (UI 구분용)
  COLOR_CHAR_NIGHT_LORD = { 0.55, 0.30, 0.85, 1.0 },
  COLOR_CHAR_ARCH_MAGE = { 0.95, 0.30, 0.25, 1.0 },
  COLOR_CHAR_PALADIN = { 0.92, 0.85, 0.22, 1.0 },
  COLOR_CHAR_ARAN = { 0.25, 0.60, 0.90, 1.0 },

  -- 초능력 게이지 UI
  CHARGE_GAUGE_W = 320,
  CHARGE_GAUGE_H = 64,
  CHARGE_GAUGE_RADIUS = 22,
  CHARGE_GAUGE_PULSE_ALPHA_MIN = 0.30,
  CHARGE_GAUGE_PULSE_ALPHA_MAX = 0.80,
  CHARGE_GAUGE_PULSE_PERIOD_SEC = 1.20
}
