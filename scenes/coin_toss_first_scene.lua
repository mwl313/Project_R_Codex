--[[
파일명: coin_toss_first_scene.lua
모듈명: CoinTossFirstScene

역할:
- local player가 선공일 때 코인 토스 연출을 보여준다.
- 연출 종료 후 자동으로 매치 씬으로 진행한다.

외부에서 사용 가능한 함수:
- CoinTossFirstScene.new(app)
]]
...
local CoinTossFirstScene = {}
CoinTossFirstScene.__index = CoinTossFirstScene

function CoinTossFirstScene.new(app)
  local instance = {
    _app = app,
    _view = nil,
  ...
  return setmetatable(instance, CoinTossFirstScene)
end

function CoinTossFirstScene:enter(params)
  ...
  self._view = CoinTossView.new({
    ...
    titleText = "선공",
    subtitleText = "당신이 선공입니다."
  })
end

function CoinTossFirstScene:update(dt)
...
function CoinTossFirstScene:draw()
  if not self._view then
    return
  end
  if self._startDelayElapsedSec < self._startDelaySec then
...
function CoinTossFirstScene:onAppEvent(event)
